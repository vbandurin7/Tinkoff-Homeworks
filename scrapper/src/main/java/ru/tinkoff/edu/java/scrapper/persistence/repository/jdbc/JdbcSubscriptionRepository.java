...
public class JdbcSubscriptionRepository implements SubscriptionRepository {
    ...
    private static final String FIND_ALL_BY_CHAT_SQL =
        "SELECT * FROM link WHERE url IN (SELECT link_url FROM chat_link WHERE chat_id = ?)";
    private static final String FIND_CHATS_BY_URL_SQL =
        "SELECT * FROM chat WHERE id IN (SELECT chat_id FROM chat_link WHERE link_url = ?)";
    ...
    private static final RowMapper<ChatDto> CHAT_ROW_MAPPER =
        (ResultSet rs, int rownum) -> new ChatDto(rs.getLong("id"));
